#!/bin/bash
#
# Pre-flight checks for the create-vcf-indexes job, run on the Jenkins agent
# BEFORE any EC2 instance is provisioned.
#
# Validates that --output is set and the target bucket is reachable.
#
# Environment:
#   TF_VAR_output_uri  required   where vcfIndex.tsv and SampleIds.csv are written
...
echo "Pre-flight: create-vcf-indexes"
echo "  output: $OUTPUT"
echo ""
...
if [[ -z "$OUTPUT" ]]; then
    fail "OUTPUT is required: the s3:// URI or local path for vcfIndex.tsv and SampleIds.csv."
fi
